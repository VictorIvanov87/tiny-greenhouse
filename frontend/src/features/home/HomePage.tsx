...
const HomePage = () => {
  ...
  return (
    <div
      ...
      style={{
        background: `linear-gradient(135deg, ${palette.sageSoft} 0%, #ffffff 35%, ${alpha(palette.sunlight, 0.22)} 100%)`,
      }}
    >
      <Card className="w-full max-w-xl space-y-5 rounded-3xl border border-[color:var(--color-evergreen-soft)] bg-[color:var(--color-card)] shadow-[0_28px_80px_var(--color-evergreen-shadow)] backdrop-blur">
        ...
        <p className="text-base" style={{ color: palette.soil70 }}>
          This placeholder dashboard is only visible while you are signed in. Use it to confirm that the route guard is
          working before live greenhouse data is wired up.
        </p>
        <Button
          color="light"
          onClick={signOut}
