...
const repoRoot = resolve(__dirname, "..", ".."); // backend/
const ragRoot = resolve(repoRoot, "..", "data", "rag"); // project-root/data/rag
	...
	if (doc.overview) {
		...
		chunkText(overviewText).forEach((chunk) =>
			results.push({
				cropId,
				lang,
				stage: doc.crop.defaultStage ?? null,
				sourcePath,
				text: chunk,
			})
		);
...
const buildMarkdownChunks = async (
	mdPath: string,
	cropId: string,
	lang: string
): Promise<RawChunk[]> => {
	...
	return chunkText(buffer).map((chunk) => ({
		cropId,
		lang,
		stage: null,
		sourcePath,
...
/**
 * Recursively discover all .yaml seed files under data/rag/crops/**.
 */
const discoverSeedFiles = async (): Promise<string[]> => {
	const seeds: string[] = [];
	const root = join(ragRoot, "crops");
	...
	const walk = async (dir: string) => {
		...
		for (const e of entries) {
			...
			if (e.isDirectory()) {
				await walk(p);
			} else if (e.isFile() && extname(e.name) === ".yaml") {
				seeds.push(p);
			}
		}
	};
 ...
 */
const collectCompanionMarkdown = async (dir: string) => {
	const all = await readdir(dir, { withFileTypes: true }).catch(() => []);
...
const main = async () => {
	...
	for (const yamlPath of seeds) {
		...
		const companions = await collectCompanionMarkdown(dirname(yamlPath));

		const mdChunks: RawChunk[] = [];
		for (const mdPath of companions) {
			mdChunks.push(
				...(await buildMarkdownChunks(
					mdPath,
					doc.crop.id,
					doc.crop.lang ?? "en"
				))
		...
		const sourcePaths = new Set<string>(
			rawChunks.map((chunk) => chunk.sourcePath)
		);
